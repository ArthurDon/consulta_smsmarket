...
class ConsultaSms:
    ...
    def url_format(self):
        ...
        url = 'https://private-anon-2815855b50-smsmarket.apiary-proxy.com/webservice-rest/mt_date?user=fssmsmarket&password=s3nh%40123%40%21%40&start_date={}&end_date={}&type=0&status=0%2C1&timezone=-03%3A00'.format(url_start_date,url_end_date)
    ...
    def return_format(self, filtered_response):
        status_dict = {'-2': 'Erro de rede da operadora',
        ...
        for element in filtered_response:
            element['status'] = status_dict.get(element.get('status'))

            format_date = datetime.datetime.strptime(element['sent_date'], "%Y-%m-%d %H:%M:%S")
